...
Operational Nomad actions not yet covered. Same rules: pure decision/render logic in `core/`, thin glue, every mutation behind the existing confirmation flow.

- [x] **NOM-34 — Scale a task group**: set a task group's `count` via `POST /v1/job/:id/scale` (with a reason), from the job panel and a tree/command entry. Mutating → explicit confirmation (a scale-down that removes allocations is destructive-ish, so it shows the delta before asking). Pure `scaleBody`/validation (target ≥ 0, group exists) in `core/scale.ts`, tested; the count picker + confirmation in the glue.
- [ ] **NOM-35 — Force a periodic job**: for a `periodic` job, `POST /v1/job/:id/periodic/force` to trigger a run now, surfaced only on periodic jobs (detected from the spec). Confirmation, then reveal the launched child job. Pure guard (`isPeriodic`) tested.
- [ ] **NOM-36 — Dispatch a parameterized job**: for a parameterized job, collect the meta keys (required/optional, from the spec) and an optional payload, then `POST /v1/job/:id/dispatch`. Pure meta-form model + validation (required keys present) in `core/dispatch.ts`, tested; the input flow + confirmation in the glue.
- [ ] **NOM-37 — Scaling status & events**: `GET /v1/job/:id/scale` → current vs desired per group and the recent scaling events (who/when/why), rendered in the job panel and as a report. Pure `renderScaleStatus` tested.
