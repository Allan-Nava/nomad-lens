...
- [ ] **NOM-32 — Node detail panel**: a webview for a node — status/eligibility/drain, its running allocations grouped by job, and drain/eligibility buttons routed through the existing confirmations. Pure `renderNodePanel` + the allow-list contract, tested.
- [ ] **NOM-33 — Global job search**: a command that lists jobs across all configured clusters in a QuickPick (fuzzy by name), and on pick switches to that cluster and opens the job panel. Pure ranking/label building tested; the cluster switch + panel open in the glue.
