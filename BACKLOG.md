...
- [ ] **NOM-28 — Live panels (push updates)**: the dashboard and job panels refresh on the deployment poll by posting an incremental state message the page applies in place (no full-HTML reload, no flicker, scroll kept), with a small "live" indicator and a `nomadLens.livePanels` opt-out. Pure state-diff/serialisation in core, tested.
- [ ] **NOM-29 — Inline resource gauges & sparklines**: the NOM-24 refinement — the job panel embeds per-task CPU/memory gauges vs requested (`progressBar`) and a sparkline of the last N samples collected over the poll window, degrading gracefully when the client node is unreachable. Sampling buffer + rendering pure and tested (reuses `core/resources.ts` + `core/webview/charts.ts`).
- [ ] **NOM-30 — Dashboard drill-down & tree reveal**: switch the tree to `createTreeView` so clicking a dashboard/job entry reveals it in the explorer and opens the job panel. Pure mapping (entry → tree element id / command) tested; the `reveal` plumbing in the glue.
