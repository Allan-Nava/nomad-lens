...
- [ ] **NOM-34 — Scale a task group**: set a task group's `count` via `POST /v1/job/:id/scale` (with a reason), from the job panel and a tree/command entry. Mutating → explicit confirmation (a scale-down that removes allocations is destructive-ish, so it shows the delta before asking). Pure `scaleBody`/validation (target ≥ 0, group exists) in `core/scale.ts`, tested; the count picker + confirmation in the glue.
